...
function App(props) {
  ...
  const handleEditTodo = (index) => {
    let newTask = prompt("Updated Task", allTodos[index]["task"]);
    if(newTask === null) return ;
    while (newTask.length === 0) {
      alert("Please enter valid task");
      newTask = prompt("Updated Task", allTodos[index]["task"]);
    }
    allTodos[index]["task"] = newTask;
