...
void light_value() {
  if (EEPROM.read(light_add)) server.send(200, "text/plain", "light 1 on");
  else server.send(200, "text/plain", "light 1 off");
}
...
void handleONTube() {
  ...
  server.send(200, "text/plain", "tube_light_on\r\n");
  light = 1;
}

void handleOFFTube() {
  digitalWrite(pinTube, HIGH);
  server.send(200, "text/plain", "tube_light_off\r\n");
  light = 0;
}
